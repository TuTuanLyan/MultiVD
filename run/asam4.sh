#!/usr/bin/env bash
# ASAM4 — truc rho, chi so CHINH khai bao truoc: ROC-AUC. Ghi vao CUNG cay `asam1_t5p`
...
SEED="${SEED:-42}"
echo "########## ASAM4 bat dau $(date -u '+%F %T') | $(hostname) ##########"
echo "  truc rho: 4.0 — duong cong PR-AUC van chua tut o 2.0 (+0.0143, 10/12), phai biet dinh o dau (doi chung 0 va 0.1 da co trong cung cay asam1_t5p)"
...
for FOLD in $FOLD_LIST; do
  for SRC in $SOURCES_LIST; do
    ...
    CONFIGS="r4p0|recadam|--sam_rho 4.0 --sam_variant asam" \
    bash run/opt1.sh
  done
done
echo "########## ASAM4 xong $(date -u '+%F %T') ##########"
