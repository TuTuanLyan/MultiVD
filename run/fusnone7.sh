...
RN="${RUN_NAME:-fusnone7}"
SD="${SEED:-7}"
BB="${BB:-codebert=microsoft/codebert-base:cls}"
STORE="model/${RN}/phase1"

exec 4>/tmp/mvd_fusnone7.lock || exit 1
flock -n 4 || { echo "DA CO fusnone7 dang chay"; exit 3; }
...
place model/p1seed/head_ad48_s7.pt codebert__latent_bottleneck_com_l0p05_ad48 || exit 5
place model/p1seed/none_ad48_s7.pt codebert__none_com_ad48                    || exit 5

# Kiem NOI DUNG checkpoint, khong chi kiem file ton tai
"$PY" - "$STORE" "$SD" <<'PYEOF' || exit 5
import torch,sys,os
store,sd=sys.argv[1],sys.argv[2]
for name,mode in [("codebert__latent_bottleneck_com_l0p05_ad48","latent_bottleneck"),
                  ("codebert__none_com_ad48","none")]:
...
echo "########## FUSNONE7 bat dau $(ts) | seed $SD | $(hostname) ##########"
...
echo "########## FUSNONE7 xong $(ts) | $(find results/${RN}_codebert -name 'fold*.json' 2>/dev/null | wc -l)/15 o ##########"
