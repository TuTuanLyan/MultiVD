#!/usr/bin/env bash
# ASAM3 — truc rho, chi so CHINH khai bao truoc: ROC-AUC. Ghi vao CUNG cay `asam1_t5p`
...
SEED="${SEED:-42}"
echo "########## ASAM3 bat dau $(date -u '+%F %T') | $(hostname) ##########"
echo "  truc rho: 0.2 va 2.0 — bit hai dau con thieu (doi chung 0 va 0.1 da co trong cung cay asam1_t5p)"
...
for FOLD in $FOLD_LIST; do
  for SRC in $SOURCES_LIST; do
    ...
    CONFIGS="r0p2|recadam|--sam_rho 0.2 --sam_variant asam
r2p0|recadam|--sam_rho 2.0 --sam_variant asam" \
    bash run/opt1.sh
  done
done
echo "########## ASAM3 xong $(date -u '+%F %T') ##########"
