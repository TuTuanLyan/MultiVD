#!/usr/bin/env bash
# P1SEED_T5P — CUNG cau hoi cua `p1seed` nhung tren backbone THU HAI (codet5p-220m-bimodal).
#
# Cong 2 cua NEXT_CONTRIBUTION doi moi co che phai lap tren CA HAI backbone truoc khi phat bieu.
# Tren codebert da do (FACTS §55.1): co head 3/3 seed hoc duoc, khong head 1/3. Muc nay hoi:
# tinh khong on dinh do co DAC THU BACKBONE khong?
#
# Pha 1 KHONG PHA 2 — khong ton mot o nao. Ket qua cung la checkpoint san sang cho khoi
# `fusnone` tren t5p (chay sau, o seed nao ma CA HAI nhanh deu lanh).
#
...
OUT="${OUT:-model/p1seed_t5p}"

exec 4>/tmp/mvd_p1seed_t5p.lock || exit 1
flock -n 4 || { echo "DA CO p1seed_t5p dang chay"; exit 3; }
PIDFILE="${PIDFILE:-/workspace/p1seed_t5p.pid}"
...
run_p1() {  # $1=aux_mode  $2=seed  $3=ten
  ...
  "$PY" -u src/train_transfer.py --phase phase1 \
    --run_name p1seed_t5p --method_name "p1_$3" \
    ...
    --num_workers 0 --model_name Salesforce/codet5p-220m-bimodal --pooling mean \
    > "log/p1seed_t5p/$3.log" 2>&1
  local rc=$?
  "$PY" - "$OUT/$3.pt" "$3" "log/p1seed_t5p/$3.log" <<'PYEOF'
import torch,sys,re
