...
ts(){ date -u '+%F %T'; }
NEED=70   # GD1: 2 bb x 5 fold x (1 baseline + 2 nguon x 2 cau hinh) = 50
          # GD2: 2 bb x 5 fold x 2 cau hinh (baseline da co) = 20
...
if flock -n /tmp/mvd_chot_chain.lock -c true 2>/dev/null; then alive=0; else alive=1; fi
...
setsid nohup bash run/chot_chain.sh >> log/chot2bb.log 2>&1 </dev/null & disown
