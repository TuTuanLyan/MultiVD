#!/usr/bin/env bash
# Watchdog cho khoi lambda=0.02 + ASAM rho=0.1 + RecAdam.  ARM=destroy bash scripts/overnight46.sh
#
...
LOG=log/overnight46.log; mkdir -p log log/vast_ntat
say(){ echo "$(date -u '+%F %T') $*" | tee -a "$LOG"; }
FAILS=0

sshx(){ timeout 30 ssh -o StrictHostKeyChecking=no -o ConnectTimeout=15 -p "$2" root@"$1" "$3" 2>/dev/null; }

say "== overnight46 bat dau, ARM=$ARM, chu ky ${EVERY}s, dung khi driver in '$DONE_RE' =="
for ((i=0;i<MAX;i++)); do
  ...
  FIN=$(grep -c "$DONE_RE" log/vast_ntat/day45_t5p.log 2>/dev/null || echo 0)
