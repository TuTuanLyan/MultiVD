#!/usr/bin/env bash
# Cai dat vast 50570168 (nhan `ntat`) va giao TOAN BO phan DAO CHIEU (Python -> JS).
...
# --- phong ---
say "phong khoi AUXB tren vast (co che head moi)"
$V "cd /workspace/MultiVD && PYTHON=$PY setsid nohup bash scripts/vast_auxb_run.sh >> log/vast_auxb.log 2>&1 </dev/null & disown"
sleep 25
$V "cd /workspace/MultiVD && tail -8 log/vast_auxb.log" | sed 's/^/    /' | tee -a "$LOG"
