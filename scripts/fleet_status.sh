...
for L in ntat ntat2; do
  ...
  out=$(timeout 60 ssh -o StrictHostKeyChecking=no -o BatchMode=yes -o ConnectTimeout=15 -p "$P" root@"$H" \
    "cd $R 2>/dev/null || exit 1
     echo \"  job=\$(ps -eo args --no-headers|grep -c '[s]rc/train_[a-z]*\.py\|[t]ools/wblend\.py') worklist=\$(ps -eo args --no-headers|grep -c '[v]ast_worklist.sh') vram=\$(nvidia-smi --query-gpu=memory.used --format=csv,noheader,nounits)MiB\"
...
echo "  job=$(ps -eo args --no-headers|grep -c '[s]rc/train_[a-z]*\.py\|[t]ools/wblend\.py') ft2=$(ps -eo args --no-headers|grep -c '[r]un/ft2.sh') vram=${used}MiB"
tail -1 log/ft2.log 2>/dev/null | sed 's/^/  ft2: /'
echo "158"
ssh -o BatchMode=yes -o ConnectTimeout=8 tranmanhcuong@112.137.129.158 \
  'echo "  job=$(ps -eo args --no-headers|grep -c "[s]rc/train_[a-z]*\.py\|[t]ools/wblend\.py") vram=$(nvidia-smi --query-gpu=memory.used --format=csv,noheader,nounits)MiB"' 2>/dev/null || echo "  khong ssh duoc"
