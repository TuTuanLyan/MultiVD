...
FAILS=0
ndriver(){ ps -eo pid,args --no-headers -u "$USER" | awk '/[d]ay45_machine\.sh/{print $1}' | wc -l; }
say "== local_watch bat dau, can codebert=$NEED o, chu ky ${EVERY}s =="
for ((i=0;i<MAX;i++)); do
