...
# VONG NGOAI. BAY DA MAC 09/09/2026 02:40 tren ntat2: worklist.txt bi GHI DE trong khi
# driver dang giu no tren fd 3, nen offset byte cua driver roi vao giua noi dung MOI ->
# no doc rac, in "WORKLIST xong | da chay 8 muc" (danh sach chi co 6) roi THOAT, trong
# khi muc `pool_cb_lm` chua bao gio chay. May vast NAM KHONG ma van tinh tien.
# Sua: het mot luot thi MO LAI file va doi chieu todo-done. Con viec thi chay luot nua.
# Chan quay vong vo han bang hai dieu kien: het luot cho phep, HOAC mot luot khong chay
# duoc muc nao (moi thu con lai deu hong/thieu file) — luc do moi thoat.
MAXPASS_WL="${MAXPASS_WL:-20}"
n=0; pass=0
while :; do
pass=$((pass+1)); ran_this_pass=0
while IFS='|' read -r SCRIPT SRCS FOLDS <&3; do
  ...
  n=$((n+1)); ran_this_pass=$((ran_this_pass+1))
done 3< "$WL"
  left=$(grep -v '^[[:space:]]*#\|^[[:space:]]*$' "$WL" | grep -vxF -f log/worklist.done | grep -c .)
  if (( left == 0 )); then
    echo "$(ts) | het viec: todo - done = 0 sau $pass luot"; break
  fi
  if (( ran_this_pass == 0 )); then
    echo "$(ts) | !! con $left muc nhung luot $pass KHONG chay duoc muc nao — CAN NGUOI XEM"
    grep -v '^[[:space:]]*#\|^[[:space:]]*$' "$WL" | grep -vxF -f log/worklist.done | sed 's/^/    con: /'
    break
  fi
  if (( pass >= MAXPASS_WL )); then
    echo "$(ts) | !! da $pass luot ma con $left muc — DUNG, can nguoi xem"; break
  fi
  echo "$(ts) | con $left muc sau luot $pass (danh sach co the da doi) — chay luot $((pass+1))"
done
echo "########## WORKLIST xong $(ts) | da chay $n muc / $pass luot ##########"
